...
function add(clmPositions) {
  if (clmPositions) {
    var foreheadWidth = posUtils.length(
      posUtils.delta(
        clmPositions[forehead.indices.start],
        clmPositions[forehead.indices.end]
      )
    );
    ...
    positions = inflatePositions(positions, foreheadWidth * 0.25);
    ...
    var centerPoint = [(minX + maxX) * 0.5, (minY + maxY) * 0.5];

    var size = [maxX - minX, maxY - minY];

    for (var i = indices.start; i <= indices.end; i++) {
      var ratio = i / length;
      var angle = ratio * 2 * Math.PI - 0.4;
      clmPositions[i][0] = centerPoint[0] + Math.cos(angle) * size[0] * 0.5;
      clmPositions[i][1] = centerPoint[1] + Math.sin(angle) * size[1] * 0.5;
    }
  }
  return clmPositions;
}
